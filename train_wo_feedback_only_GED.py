...
from model.net2_wo_feedback2 import Net
...
# parser.add_argument("--model_path", type=str, default="./logs/Ver.4.27.net2/", help='')  # net
# parser.add_argument("--save_path", type=str, default="./logs/Ver.7.11.net2.Results", help='') 
parser.add_argument("--save_path", type=str, default="./logs/Ver.net2_wo_feedback_only_GED", help='')  # 88.95
...
for epoch in range(opt.epochs):
    ...
    for idx, (input_data, glass, ghost, r1, r2, h, w, with_shift) in enumerate(loader_train, 0):
        ...
        lossH = shift_loss(h3, h2, h1, h0, h, with_shift)
        
        lossW = shift_loss(w3, w2, w1, w0, w, with_shift)
        ...
        loss = lossR1 + lossR2 + 4 * lossGhost + 0.1 * lossPer + 0.1 * lossW + 0.1 * lossH
        loss.backward()
    ...
    print('INFO: epoch:{},tl:{},tpl:{},twl:{},thl:{},tr1:{},tr2:{},tgs:{},tgl:{},V_gh:{}, time:{}'.format(
        epoch + 1,
        ...
        round(v_ghost_iou / len(
            loader_valid) / opt.batchSize * 100, 2), round(t, 2)))
