...
class GDM(nn.Module):
    ...
    def forward(self, x):
        ...
        merge = self.merge(torch.cat([r1_mask, r2_mask], 1))
...
class Net(nn.Module):
    def __init__(self, backbone_path=None, pred_channel=48):
        ...
        GED_swin = SwinTransformer_demo(img_size=384, embed_dim=128, depths=[2, 2, 2, 2],
    ...
    def forward(self, x):
        ...
        layer3_f = ged_layer3
        merge3, r1_3, r2_3 = self.GDM3(layer3_f)
        layer3_f = merge3 * layer3_f
        p32 = self.ged_up_32(layer3_f)

        layer2_f = p32 + ged_layer2
        merge2, r1_2, r2_2 = self.GDM2(layer2_f)
        layer2_f = merge2 * layer2_f
        p21 = self.ged_up_21(layer2_f)

        layer1_f = p21 + ged_layer1
        merge1, r1_1, r2_1 = self.GDM1(layer1_f)
        layer1_f = merge1 * layer1_f
        p10 = self.ged_up_10(layer1_f)

        layer0_f = p10 + ged_layer0
        merge0, r1_0, r2_0 = self.GDM0(layer0_f)
        layer0_f = merge0 * layer0_f
