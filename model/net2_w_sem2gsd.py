...
#            SEM_Net_demo.load_state_dict(new_state_dict, strict=False)
        ...
        self.ghost_final_pred = nn.Conv2d(4, 1, 3, 1, 1)
        self.shift_final_pred = nn.Conv2d(8, 2, 3, 1, 1)
        ...
        if backbone_path is not None:
            ...
            new_first_layer = torch.nn.Conv2d(6, 128, 4, 4)
    ...
    def forward(self, x):
        ...
        ged_fuse_feature = torch.cat((ged_layer0_pred, ged_layer1_pred, ged_layer2_pred, ged_layer3_pred), dim=1)
        ged_final_pred = self.ghost_final_pred(ged_fuse_feature)
        
        shift_fuse_feature = torch.cat((out0, out1, out2, out3), dim=1)
        shift_final_pred = self.shift_final_pred(shift_fuse_feature)

        # GSD
        ghost = ged_final_pred
        shift = shift_final_pred
        x = torch.cat((input, ghost, shift), dim=1)
        b, c, h, w = x.shape
        x = self.pebed(x)
        x = self.pos_drop(x)
        ...
        h, w = torch.split(shift_final_pred, [1, 1], 1)
