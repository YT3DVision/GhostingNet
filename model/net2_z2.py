...
#            SEM_Net_demo.load_state_dict(new_state_dict, strict=False)
        ...
        if backbone_path is not None:
            state_dict = torch.load(backbone_path)
            pretrained_dict = state_dict['model']
            # 预训练模型中swin的position-embedding的通道是4，与调整网络后的通道数不符，此时删除position-embedding
            ...
            new_first_layer = torch.nn.Conv2d(8, 128, 4, 4)
    ...
    def forward(self, x):
        ...
        np.save(os.path.join(r"E:\PycharmProjects\GDD\train\gedfusionnpy_new2", self.img_names[self.count][:-4] + ".npy"), ged_swin_encode_feature_fusion.squeeze(0).cpu())
        self.count += 1
        x = torch.cat((input, ghost, ged_swin_encode_feature_fusion), dim=1)
        b, c, h, w = x.shape
        x = self.pebed(x)
        x = self.pos_drop(x)
...
if __name__ == '__main__':
    # img_name = os.listdir(r"E:\PycharmProjects\GDD\train\image")
