...
class Net(nn.Module):
    def __init__(self,
                 ...
                 scale=384,
                 img_path=None,
                 generate_fea4other_dataset=False,
                 mode=None):
        super(Net, self).__init__()
        self.scale = scale
        self.generate_fea4other_dataset = generate_fea4other_dataset
        self.mode = mode
        if generate_fea4other_dataset:
            self.img_names = os.listdir(img_path)
            self.img_names.sort(key=lambda e: int(e.split('.')[0]))
            self.count = 0
    ...
    def forward(self, x):
        ...
        """GED编码器的输出用于指导GSD编码"""
        ged_e0, ged_e1, ged_e2, ged_e3 = ged_layer0, ged_layer1, ged_layer2, ged_layer3
        ...
        if self.generate_fea4other_dataset:
            save_path = os.path.join(r"E:\PycharmProjects\GDD", self.mode, "ged_skip_connect_fea")
            np.save(os.path.join(save_path, str(self.img_names[self.count][:-4]) + "_0.npy"), ged_e0.squeeze(0).cpu())
            np.save(os.path.join(save_path, str(self.img_names[self.count][:-4]) + "_1.npy"), ged_e1.squeeze(0).cpu())
            np.save(os.path.join(save_path, str(self.img_names[self.count][:-4]) + "_2.npy"), ged_e2.squeeze(0).cpu())
            np.save(os.path.join(save_path, str(self.img_names[self.count][:-4]) + "_3.npy"), ged_e3.squeeze(0).cpu())
            self.count += 1

        f0 = self.fusion0(x, ged_e0)
        layer0, layer0_d = self.layer0(f0)

        f1 = self.fusion1(layer0_d, ged_e1)
        layer1, layer1_d = self.layer1(f1)

        f2 = self.fusion2(layer1_d, ged_e2)
        layer2, layer2_d = self.layer2(f2)

        f3 = self.fusion3(layer2_d, ged_e3)
