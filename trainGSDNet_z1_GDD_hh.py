...
from model.net2_z1_GSD import Net as GSDNet
from model.net2_z1 import Net as GEDNet
from config import *
from utils.load_GDD_net2_z1 import *
...
parser.add_argument("--ged_model_path", type=str, default=r"E:\PycharmProjects\GhosetNetV3\logs\V2023629_net_z\glass_max.pth", help='')
parser.add_argument("--save_path", type=str, default="./logs/GSDNet/net2_z1_GDD", help='')
...
def infer():
    ...
    for var in file_list:
        img_path = os.path.join("E:\PycharmProjects\GDD", var, "image")
        ged_model = GEDNet(backbone_path=swin_B, mode=var, img_path=img_path, generate_fea4other_dataset=True).to(device)
        ged_model.load_state_dict(torch.load(opt.ged_model_path))
        ged_model.eval()

        # 2 check
        train_data_path = os.path.join(opt.data_path, var)
        num_img = len(os.listdir(os.path.join(train_data_path, "image")))
        ghost_npy_train_path = os.path.join(train_data_path, "ghostnpy_z1")
        if os.path.exists(ghost_npy_train_path) and len(os.listdir(ghost_npy_train_path)) == num_img:
            return

        # 3 generate
        os.makedirs(ghost_npy_train_path)
        gedfusion_npy_train_path = os.path.join(train_data_path, "ged_skip_connect_fea")
        os.makedirs(gedfusion_npy_train_path)

        with torch.no_grad():
            generate_npy(ghost_npy_train_path, ged_model)


def generate_npy(ghost_path, ged_model):
    var = "train" if "train" in ghost_path else "test"
    name_list = os.listdir(os.path.join(opt.data_path, var, "image"))
    name_list.sort(key=lambda e: int(e.split('.')[0]))
    for idx, (img_name) in enumerate(tqdm(name_list)):
...
def train():
    ...
    print(f"Current Net: {gsd_model._get_name()}")
    ...
    for epoch in range(opt.epochs):
        ...
        for idx, (image, glass, ghost, gscf0, gscf1, gscf2, gscf3) in enumerate(tqdm(loader_train)):
            image = image.to(device)
            glass = glass.to(device)
            ghost = ghost.to(device)
            gscf0 = gscf0.to(device)
            gscf1 = gscf1.to(device)
            gscf2 = gscf2.to(device)
            gscf3 = gscf3.to(device)

            optimizer.zero_grad()

            l3, l2, l1, l0, final_glass = gsd_model(image, ghost, gscf0, gscf1, gscf2, gscf3)
        ...
        with torch.no_grad():
            for idx, (image, glass, ghost, gscf0, gscf1, gscf2, gscf3) in enumerate(tqdm(loader_valid)):
                image = image.to(device)
                glass = glass.to(device)
                ghost = ghost.to(device)
                gscf0 = gscf0.to(device)
