...
parser.add_argument("--model_path", type=str, default="./ablation/GED/ghost_max.pth", help='') 
parser.add_argument("--save_path", type=str, default="./ablation/GED2", help='') 
...
loader_train = DataLoader(dataset=dataset_train, num_workers=8, batch_size=opt.batchSize, shuffle=True, drop_last=True)
loader_valid = DataLoader(dataset=dataset_valid, num_workers=8, batch_size=opt.batchSize, shuffle=False)
...
for epoch in range(opt.epochs):
    ...
    for idx, (input_data, glass, ghost, r1, r2, h, w, with_shift) in enumerate(loader_train, 0):
        ...
        loss = lossR1 + lossR2 + 4 * lossGhost + 0.1 * lossPer + 0.01 * lossW + 0.01 * lossH
        loss.backward()
