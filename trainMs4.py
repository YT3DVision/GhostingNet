...
parser.add_argument("--save_path", type=str, default="./logs/Ver4.18", help='')  # 88.95
...
for epoch in range(opt.epochs):
    ...
    for idx, (input_data, glass, ghost, r1, r2) in enumerate(loader_train, 0):
        ...
        lossr13 = bce_ssim_loss(r1_3, r1)
        lossr12 = bce_ssim_loss(r1_2, r1)
        lossr11 = bce_ssim_loss(r1_1, r1)
        lossr10 = bce_ssim_loss(r1_0, r1)

        lossR1 = lossr13 + lossr12 + lossr11 + lossr10

        lossr23 = bce_ssim_loss(r2_3, r2)
        lossr22 = bce_ssim_loss(r2_2, r2)
        lossr21 = bce_ssim_loss(r2_1, r2)
        lossr20 = bce_ssim_loss(r2_0, r2)

        lossR2 = lossr23 + lossr22 + lossr21 + lossr20

        lossg3 = bce_ssim_loss(g3, ghost)
        lossg2 = bce_ssim_loss(g2, ghost)
        lossg1 = bce_ssim_loss(g1, ghost)
        lossg0 = bce_ssim_loss(g0, ghost)
        lossgfuse = bce_ssim_loss(fg, ghost)
        ...
        loss = lossR1 + lossR2 + 3 * lossGlass + 2 * lossGhost + 0.1 * lossPer
        loss.backward()
    ...
    with torch.no_grad():
        for idx, (input_data, glass, ghost) in enumerate(loader_valid, 0):
            input_data = Variable(input_data)
            ghost = Variable(ghost)
            glass = Variable(glass)
            r1 = Variable(r1)
    ...
    print('INFO: epoch:{},tl:{},tpl:{},tr1:{},tr2:{},tgs:{},tgl:{},V_gh:{},V_gl:{}, time:{}'.format(epoch + 1,
