...
from model.GhostingNetH2 import GhostingNet
...
parser.add_argument("--save_path", type=str, default="./logs/Ver.w_r1--3", help='')  # 88.95
...
# ##########create model###############
model = GhostingNet(backbone_path=swin_B)
if opt.use_GPU:
    model = model.cuda()
...
for epoch in range(opt.epochs):
    ...
    for idx, (input_data, glass, ghost, r1, r2, h, w, with_shift) in enumerate(loader_train, 0):
        ...
        loss = lossR1 + 2 * lossGlass + 4 * lossGhost 
        loss.backward()
