...
class DCN_sep(DCNv2):
    ...
    def forward(self, input, fea):
        """
        input: input features for deformable conv (此处是r1)
        fea: other features used for generating offsets and mask (此处是r1+r2)
        """
        out = self.conv_offset_mask(fea)
        o1, o2, mask = torch.chunk(out, 3, dim=1)
        # 通过真值的约束学到了一个θ
        offset = torch.cat((o1, o2), dim=1)
        # 对卷积核坐标进行限制，防止偏移量过大移出feature map的范围
        # transforms.ToPILImage()(F.sigmoid(torch.chunk(offset.squeeze(0), 144, dim=0)[122])).save(r"E:\PycharmProjects\GhosetNetV3\utils\a12_theta_122.jpg")

        # torchvision.transforms.ToPILImage()(torch.nn.functional.sigmoid(torch.chunk(offset.squeeze(0), 144, dim=0)[143])).show()
        ...
        mask = torch.sigmoid(mask)
        # r1平移θ能到r2，通过Loss来(约束)实现它。确保是平移得到，而非相似或仿射变换等其他操作得到。
