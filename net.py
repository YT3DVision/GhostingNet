...
#            SEM_Net_demo.load_state_dict(new_state_dict, strict=False)
        ...
        self.ged_down3 = nn.Conv(1024 * 2 + 2, 1024, 1, 1, 0)
        self.ged_down2 = nn.Conv(512 * 2 + 2, 512, 1, 1, 0)
        self.ged_down1 = nn.Conv(256 * 2 + 2, 256, 1, 1, 0)
        self.ged_down0 = nn.Conv(128 * 2 + 2, 128, 1, 1, 0)
        ...
        self.f_conv = nn.Conv2d(4, 3, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        layer3_f = self.ged_down(torch.cat((layer3_f, s3), dim=1))
        ...
        layer2_f = self.ged_down(torch.cat((layer2_f, s2), dim=1))
        ...
        layer1_f = self.ged_down(torch.cat((layer1_f, s1), dim=1))
        ...
        layer0_f = self.ged_down(torch.cat((layer0_f, s0), dim=1))
