...
parser.add_argument("--data_path", type=str, default="/data/dataSet/GEGD2", help='')
parser.add_argument("--save_path", type=str, default="./logs/Ver4.22_Finutune985", help='')  # 88.95
...
# ##########create model###############
model = Net()
state_dict = torch.load('/data/Nips/Ver1.1/logs/Ver4.21_shift2/train_min.pth')
...
for name, module in model._modules.items():
    # print(name)
    if name != "SEM3" and name != "SEM2" and name != "SEM1" and name != "SEM0" and name != "h_final_pred" and name != "w_final_pred":
...
def shift_loss(s3, s2, s1, s0, s, gt, with_shift):
    ...
    s = torch.sum(torch.mean(((s - gt)**2).view(with_shift.numel(), -1), 1) * with_shift)
    loss = s3 + s2 + s1 + s0 + s
    return loss
...
for epoch in range(opt.epochs):
    ...
    for idx, (input_data, glass, ghost, r1, r2, h, w, with_shift) in enumerate(loader_train, 0):
        ...
        lossH = shift_loss(h3, h2, h1, h0, hf, h, with_shift)
        lossW = shift_loss(w3, w2, w1, w0, wf, w, with_shift)
