...
class Net(nn.Module):
    def __init__(self,
        ...
        if backbone_path is not None:
            state_dict = torch.load('/data/Nips/Ver1.1/logs/Ver1.2/train_min.pth')
            ...
            for k, v in state_dict.items():
                ...
                if name == 'SEM':
                    new_state_dict[k] = v
            print("---start load pretrained modle of swin encoder---")
            SEM_Net_demo.load_state_dict(pretrained_dict, strict=False)

        self.SEM3 = SEM_Net_demo.SEM3
        self.SEM2 = SEM_Net_demo.SEM2
        self.SEM1 = SEM_Net_demo.SEM1
        self.SEM0 = SEM_Net_demo.SEM0

        self.SEM3.requires_grad_(False)
        self.SEM2.requires_grad_(False)
        self.SEM1.requires_grad_(False)
        self.SEM0.requires_grad_(False)
