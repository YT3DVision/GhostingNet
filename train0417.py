...
parser.add_argument("--save_path", type=str, default="./logs/Ver4.14", help='')  # 88.95
...
def shift_loss(s3, s2, s1, s0, gt, with_shift):
    ...
    s1 = torch.sum(torch.mean(((s2 - gt)**2).view(with_shift.numel(), -1), 1) * with_shift)
    s0 = torch.sum(torch.mean(((s2 - gt)**2).view(with_shift.numel(), -1), 1) * with_shift)
    loss = s3 + s2 + s1 + s0
    return loss
...
for epoch in range(opt.epochs):
    ...
    for idx, (input_data, glass, ghost, r1, r2, h, w, with_shift) in enumerate(loader_train, 0):
        ...
        loss = lossR1 + lossR2 + 3 * lossGlass + 3 * lossGhost + 0.1 * lossPer + lossW + lossH
        loss.backward()
